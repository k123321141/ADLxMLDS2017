...
if __name__ == '__main__':
    ...
    model.compile(loss=loss.loss_with_mask, optimizer=opt,metrics=[loss.acc_with_mask,'accuracy'],sample_weight_mode = 'temporal')
    early_stopping = EarlyStopping(monitor='val_loss', patience=3)
    #make check points to trace the performance of model during training
    cks = ModelCheckpoint('../tail.{epoch:02d}-{val_loss:.2f}.cks',save_best_only=True,period = 1)
