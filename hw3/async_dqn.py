...
flags.DEFINE_string('game', 'BreakoutNoFrameskip-v4', 'Name of the atari game to play. Full list here: https://gym.openai.com/envs#atari')
...
def actor_learner_thread(thread_id, env, session, graph_ops, num_actions, summary_ops, saver):
    ...
    env = Environment(FLAGS.game, parse(), atari_wrapper=True)
...
def train(session, graph_ops, num_actions, saver):
    ...
    envs = [Environment(FLAGS.game, parse(), atari_wrapper=True) for i in range(FLAGS.num_concurrent)]
...
def main(_):
  ...
  with g.as_default(), tf.Session(config = config) as session:
