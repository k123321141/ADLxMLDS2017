...
def model_generator():
    ...
    model.add(Convolution2D(int(nch / 8), h, h, border_mode='same', W_regularizer=reg()))
    model.add(BatchNormalization(mode=0, axis=1))
    model.add(LeakyReLU(0.2))
    model.add(UpSampling2D(size=(2, 2)))
...
def model_discriminator():
    ...
    c0 = Convolution2D(int(nch / 8), h, h, border_mode='same', W_regularizer=reg(),
                       input_shape=dim_ordering_shape((3, 64, 64)))
    '''
...
def example_gan(adversarial_optimizer, path, opt_g, opt_d, nb_epoch, generator, discriminator, latent_dim,
    ...
    for e in range(1000):
        generator_cb = ImageGridCallback(os.path.join(path, "epoch-%d.png" % e), generator_sampler, cmap=None)
        callbacks = [generator_cb]
        history = fit(model, x=xtrain, y=y, validation_data=(xtest, ytest),
                      callbacks=callbacks, nb_epoch=1,
                      batch_size=32)
        # save models
        print ''
        generator.save(os.path.join(path, "generator_%d.h5" % e))
        discriminator.save(os.path.join(path, "discriminator_%d.h5" % e))
...
def main():
    ...
    example_gan(AdversarialOptimizerSimultaneous(), "output/gan-cifar10",
                ...
                nb_epoch=100000, generator=generator, discriminator=discriminator,
